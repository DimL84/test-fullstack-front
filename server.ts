...
export class Server {
  ...
  #httpConnections: Set<Deno.TcpConn> = new Set();
  #onError: (error: unknown) => Response | Promise<Response>;
   ...
   */
  async #respond(
    requestEvent: Deno.HttpRequestEvent,
    connInfo: ConnInfo,
  ) {
   ...
   * @param connInfo Information about the underlying connection.
   */
  async #serveHttp(httpConn: Deno.TcpConn, connInfo: ConnInfo) {
    while (!this.#closed) {
   ...
   */
  #closeHttpConn(httpConn: Deno.TcpConn) {
   ...
   */
  #trackHttpConnection(httpConn: Deno.TcpConn) {
   ...
   */
  #untrackHttpConnection(httpConn: Deno.TcpConn) {
